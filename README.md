...
Synthetic RNA designed by a model is not yet a molecule. It still has to be the cassette it claims to be, and its coding sequence still has to satisfy the grammar of translation. RNA Lab writes that evaluation beside the polymer. The package `rna` (CLI `rn01`) accepts four slots, leaves `slots.*.seq` as submitted, and returns `stamps`: identity against packaged `auth.seq` when a cassette is named, open-reading-frame grammar on the CDS, cryptic-ORF excludes on the UTRs, and composition plus fold that do not decide.
...
Identity is exact equality of a filled slot with the packaged `auth.seq` for a `claimed_id`. A cassette with the right Kozak substring and the wrong 5′ UTR is not cassette A. Substring search is not identity. The longest AUG-to-stop ORF in an unsplit polymer is not the payload.

This is a sequence-level evaluation. It does not replace in-vitro transcription, cap analysis, integrity electrophoresis, cellular expression, or a folding engine such as ViennaRNA. Reported free energies are pair-count enthalpies, not Turner nearest-neighbour MFE. Structure is measured. It does not set the stamp.

## Model details

RNA Lab is the Python package `rna`, version 0.2.0. It is a rule-based evaluator for synthetic RNA, with no runtime dependencies, released under MIT. Each call returns a JSON document of schema `rna.card.v1` with `slots` and `stamps`. Packaged identities live in `rna.auth` as `auth/<claimed_id>.fa`. The intended host for codon statistics is *Homo sapiens*; other organisms may be named and are not rescored.

## Intended use

The primary use is to evaluate synthetic mRNA designs produced by models that operate on biological sequence, before anyone spends an in-vitro transcription reaction. When `claimed_id` is set, the stamp answers whether the polymer is the named cassette. When it is not set, the stamp answers whether open-reading-frame grammar is satisfied. A human operator can submit the same document.
...
Input is `slots.5_utr.seq`, `slots.cds.seq`, `slots.3_utr.seq`, and `slots.polya.seq`. Those strings are stored as submitted. Stamps never overwrite them. Optional `claimed_id` names a packaged cassette. Metadata (name, model, class, host, product, declared chemistry, purpose) may sit beside the slots.

A raw sequence or FASTA is accepted when the slots are empty. DNA T is transcribed to U. An unsplit polymer does not invent a CDS. If the caller also supplies explicit CDS bounds, the polymer may be sliced into slots; there is no ORF search.

Class `mRNA` invokes CDS grammar on the CDS slot. Any other class skips that grammar. Motifs that must occur, motifs that must not occur, and the default leftover set (T7 promoter, poly(G)₈, poly(C)₉, BsaI, BsmBI) remain manufacturing constraints. They are not identity.

Two fixture cards ship in `cards/`: `gold.json` (claim A, polymer A) and `idiot.json` (claim A, 5′ UTR is B).

## Evaluation

Each slot is stamped with `identity: true|false` and a result of `match`, `cleared`, or `fail`.
